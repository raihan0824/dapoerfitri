...
const Contacts = ({ info }) => {
  ...
  return (
  <div data-aos="zoom-down" className={cn('app_bg', 'app_wrapper', 'section_padding')} id='contact'>
    <div className={cn('app_container', 'app_wrapper')}>
      <div data-aos="fade-up" className={cn(styles.contact_info, 'app_wrapper_info')}>
        {/* <SubHeading title={info?.metadata?.section} /> */}
        <h1 className='headtext_cormorant'>{info?.metadata?.title}</h1>
        <div data-aos="fade-up" className={cn(styles.contact_title, 'app_wrapper_content')}>
        ...
        <div data-aos="fade-up" className={styles.contact_social}>
          <SocialMedia />
        </div>
      </div>
